...
function changeWeather()
	...
	fetchRemote("https://api.openweathermap.org/data/2.5/weather?q=Budapest,HU&APPID=7fa8c822425802dd177301ff75674379",
		function(data)
			local result = fromJSON(data)
			local temp_c = result['main'].temp
			local weather = tonumber(result["weather"])
			...
			outputDebugString("Időjárás: " .. weatherNames[weather] .. ", Hőmérséklet: ".. temp_c .." °C", 0, 100, 100, 100)
			outputChatBox("#CFE2F1[✸ Időjárás jelentés]#ffffff Jelenlegi időjárás: #5C9CD5".. weatherNames[weather] .. "#ffffff | Hőmérséklet: #5C9CD5" .. math.round(temp_c)/10 .. "#ffffff°C", root, 255, 255, 255, true)
		end,
	nil, true)
end
...
addCommandHandler("timeset",
	function (player, command, hour, minute)
		--if getElementData(player, "admin >> level") > 8 then
			if not hour then
				outputChatBox("[IDŐJÁRÁS] /" .. command .. " [óra (* = valós idő)] [perc]", player, 255,255,255, true)
			elseif hour == "*" then
				setServerTime(false, false, false)
				outputChatBox("[IDŐJÁRÁS] Az idő sikeresen visszaállítva a valós időhöz.", player, 255,255,255, true)
			else
				hour = tonumber(hour) or 12
				minute = tonumber(minute) or 0

				if hour < 0 or hour > 23 then
					outputChatBox("[IDŐJÁRÁS] Az óra nem lehet kisebb mint #ff46460 #ffffffés nem lehet nagyobb mint #ff464623#ffffff.", player, 255,255,255, true)
					return
				end

				if minute < 0 and minute > 59 then
					outputChatBox("[IDŐJÁRÁS] A perc nem lehet kisebb mint #ff46460 #ffffffés nem lehet nagyobb mint #ff464659#ffffff.", player, 255,255,255, true)
					return
				end

				setServerTime(hour, minute, true)
				outputChatBox("Az idő sikeresen átállítva! #ffa600(" .. string.format("%.2i:%.2i", hour, minute) .. ")", player, 255,255,255, true)
			end
		--end
	end
)

addCommandHandler("weatherset",
	function (player, command, weatherId)
		--if getElementData(player, "admin >> level") > 8 then
			if not weatherId then
				outputChatBox("[IDŐJÁRÁS] /" .. command .. " [időjárás id (* = random)]", player, 255,255,255, true)
			elseif weatherId == "*" then
				setServerWeather(false, false)
			else
				weatherId = tonumber(weatherId) or 0
				setServerWeather(weatherId, true)
				outputChatBox("[IDŐJÁRÁS] Az időjárás sikeresen átállítva! #ffa600(" .. weatherId .. ")", player, 255,255,255, true)
			end
		--end
	end
)