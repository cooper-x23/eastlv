		...
		if hand.show == true then
			...
			if veh then
				if getElementSpeed (veh) < 5 then
					dxDrawImage (hand.x,hand.y,65,275,"handbrake/bg.png", 0, 0, 0, tocolor(0,0,0,220))
					dxDrawImage(hand.x,hand.currentY,64,16,"handbrake/line.png", 0,0,0, tocolor(0,0,0,150))
				end
			else
				showHandbrakeGui (false)
			end
...
function informatePlayerAboutHandbrake ()
	...
	if playerVeh then
		if getVehicleEngineState (playerVeh) == true then
			outputChatBox ("A kézifék be van húzva.", 255,255,255,true)
		end
	end
end
